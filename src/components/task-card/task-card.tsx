...
export const TaskCard: FC<TaskCardProps> = props => {
  const { setIsCreatingMode, isCreateMode = false, tasks, setTasks, text, criteria, format, maxScore, index, taskAssets } = props;
  ...
  return (
    <div className={DEFAULT_CLASSNAME}>
          ...
          <div className={`${DEFAULT_CLASSNAME}_new-asset`}>
              <div className={`${DEFAULT_CLASSNAME}_new-asset_content`}>
                  <div className={`${DEFAULT_CLASSNAME}_new-asset_content_image`}>
                      ...
                      {!newAssetImage && <Typography className={`${DEFAULT_CLASSNAME}_new-asset_content_image-title`}>Загрузить картинку</Typography>}
                  ...
                  <div className={`${DEFAULT_CLASSNAME}_new-asset_buttons`}>
                      <button onClick={closeNewAssetHandler}><CloseIcon /></button>
                      <button onClick={saveNewAssetHandler}><CheckIcon /></button>
                  </div>
              </div>
          </div>
      }
      ...
      <div className={`${DEFAULT_CLASSNAME}_upper`}>
        <div className={`${DEFAULT_CLASSNAME}_task`}>
          <div className={`${DEFAULT_CLASSNAME}_task-container`}>
            <div className={`${DEFAULT_CLASSNAME}_task-container_title`}>Задание {isCreateMode ? tasks.length! + 1 : index}</div>
            <div className={`${DEFAULT_CLASSNAME}_task-container_content`}>
              {isCreateMode && <textarea placeholder={"Текст задания"} value={taskText} onChange={(e) => setTaskText(e.currentTarget.value)} />}
          ...
          <div className={`${DEFAULT_CLASSNAME}_task_score`}>
            <div className={`${DEFAULT_CLASSNAME}_task_score_maxScore`}>
              <div className={`${DEFAULT_CLASSNAME}_task_score_maxScore-title`}>Максимальный балл за задание</div>
              {isCreateMode && <input placeholder={"10"} value={taskMaxScore} type={"text"} onChange={(e) => setTaskMaxScore(e.currentTarget.value)} />}
        ...
        <div className={`${DEFAULT_CLASSNAME}_criteria`}>
          <div className={`${DEFAULT_CLASSNAME}_criteria-text`}>
            ...
            <div className={`${DEFAULT_CLASSNAME}_task-container_content`}>
              {isCreateMode && <textarea placeholder={"Критерии задания"} value={taskCriteria} onChange={(e) => setTaskCriteria(e.currentTarget.value)} />}
              {!isCreateMode && <Typography>{criteria}</Typography>}
            </div>
          </div>
          {isCreateMode && <div className={`${DEFAULT_CLASSNAME}_criteria-attach`} onClick={() => addNewAssetHandler()}><AttachIcon /></div>}
        </div>
      </div>
      ...
      {!isCreateMode && <div className={`${DEFAULT_CLASSNAME}_trash`} onClick={deleteTaskHandler}><TrashIcon /></div>}
      {isCreateMode && <div className={`${DEFAULT_CLASSNAME}_save`} onClick={saveNewTaskHandler}><CheckIcon /></div>}
    </div>
  )
}
