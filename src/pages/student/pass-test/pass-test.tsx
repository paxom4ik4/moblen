...
export const PassTest: FC = () => {
  ...
  const initialTasks = useMemo(() => tasks.map(task => ({
    ...task,
    answer: "",
  })), [tasks]);

  const [tasksWithStudentAnswers, setTasksWithStudentAnswers] = useState<TaskWithAnswer[]>(initialTasks);
  ...
  return (
    <div className={DEFAULT_CLASSNAME}>
      ...
      <div className={`${DEFAULT_CLASSNAME}_tasks`}>
        ...
        <button className={`${DEFAULT_CLASSNAME}_submit-test`} onClick={submitTestHandler}><CheckIcon /></button>
      </div>
    </div>
  )
}
