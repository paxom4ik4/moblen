  ...
  return (
    <div className={DEFAULT_CLASSNAME}>
            ...
            courses.map((course) => (
              <div
                ...
                className={`${DEFAULT_CLASSNAME}_subjects_list-item ${
                  course === activeCourse && 'active-subject'
                }`}>
                <Typography color={course === activeCourse ? 'purple' : 'default'}>
                  {course}
                </Typography>
      ...
      <div className={`${DEFAULT_CLASSNAME}_tasks`}>
        ...
        {activeTopic &&
          !!activeTopic?.tests?.length &&
          activeTopic.tests.map(() => <TestCard id={'test-id'} setTestToShare={setTestToShare} />)}
        {activeTopic && (
          <button onClick={() => handleTestCreate()} className={`${DEFAULT_CLASSNAME}_tasks_add`}>
            {' '}
            <AddIcon />{' '}
          </button>
        )}
