...
const groupsAPI = {
  ...
  editGroupName: ({ groupId, groupName }: { groupId: string, groupName: string }) => {
    return API.patch(`${GROUPS_API_URL}/${groupId}/`, { group_name: groupName }).then(res => res.data)
  },
  ...
  addNewStudent: ({ groupId, studentId }: { groupId: string, studentId: string }) => {
    return API.post(`${GROUPS_API_URL}/${groupId}/new-student/`, { student_uuid: studentId })
      .then(res => res.data);
  }
};
